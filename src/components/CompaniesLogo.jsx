  ...
  return (
    <div>
      <div className="container">
        ...
        <div className="flex pt-[63px] max-md:pt-10 max-lg:flex-wrap max-lg:justify-center">
          {LOGOS.map((LOGO, index) => (
            <img
              key={index}
              className={`pointer-events-none max-w-[126.67px] h-[62px] ${index !== LOGOS.length - 1 ? 'mr-[51px] max-xl:mr-[30px] max-lg:mr-[38px]' : ''
                } ${index >= 2 ? 'max-md:mt-10' : ''}`}
              src={LOGO.src}
              alt={LOGO.alt}
            />
          ))}
